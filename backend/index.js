...
app.get('/api/registros', async (req, res) => {
  ...
  try {
    // Return registros joined with tipos_vehiculo to include a readable `tipo` string
    let sql = `SELECT r.*, t.nombre as tipo FROM registros r LEFT JOIN tipos_vehiculo t ON r.TIPOS_VEHICULO_id = t.id`;
    const params = [];
    if (estado) { sql += ' WHERE r.estado = ?'; params.push(estado); }
    sql += ' ORDER BY r.id DESC LIMIT 500';
    const [rows] = await pool.query(sql, params);
    // Normalize tipo to lowercase string when available
    const out = (rows || []).map(r => ({ ...r, tipo: r.tipo ? String(r.tipo).toLowerCase() : undefined }));
    res.json(out);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/registros', async (req, res) => {
  try {
    ...
    res.json({ id: result.insertId, placa, fecha, estado: 'EN_CURSO', TIPOS_VEHICULO_id: tipoId || null, ESPACIOS_id: espacioId, usuario_entrada_id: usuarioId, TARIFAS_id: tarifaId });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
